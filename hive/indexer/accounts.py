...
class Accounts:
    ...
    def update_follows(cls, accounts):
        ...
        sql = """
            UPDATE hive_accounts
               SET followers = (SELECT COUNT(*) FROM hive_follows2 WHERE state = 1 AND following = hive_accounts.id),
                   following = (SELECT COUNT(*) FROM hive_follows2 WHERE state = 1 AND follower  = hive_accounts.id)
             WHERE id IN :ids
        """
